...
BEGIN
    ...
    CREATE TABLE ad_view_building_result(tag VARCHAR(55));
...
BEGIN
    ...
    GROUP BY foodName
    ORDER BY
        CASE WHEN (i_sortedBy = "foodName" AND (i_sortDirection = "ASC" OR i_sortDirection IS NULL)) THEN foodName END ASC,
...
DROP PROCEDURE IF EXISTS mn_filter_foodTruck; -- Rose's procedure
DELIMITER //
...
BEGIN
     ...
     CREATE TABLE mn_filter_foodTruck_result(foodTruckName varchar(100), stationName varchar(100),
		remainingCapacity int, staffCount int, menuItemCount int) AS 
(SELECT FOODTRUCK.foodTruckName, FOODTRUCK.stationName, (STATION.capacity - (SELECT COUNT(stationName) FROM foodtruck WHERE foodtruck.stationName = i_stationName)) as remainingCapacity, COUNT(DISTINCT username) as staffCount, COUNT(DISTINCT foodName) as menuItemCount
    FROM FOODTRUCK
    INNER JOIN STATION
    ...
    mn_filter_foodtruck_result((i_hasRemainingCapacity = TRUE AND (STATION.capacity - (SELECT COUNT(stationName) FROM foodtruck WHERE foodtruck.stationName = i_stationName))>0) OR (i_hasRemainingCapacity = FALSE))
    GROUP BY foodTruckName
    HAVING 
    ((i_minStaffCount IS NULL AND i_maxStaffCount IS NULL) OR (i_minStaffCount IS NULL AND COUNT(DISTINCT username) <= i_maxStaffCount) OR (i_maxStaffCount IS NULL AND i_minStaffCount <= COUNT(DISTINCT username)) OR (COUNT(DISTINCT username) BETWEEN i_minStaffCount AND i_maxStaffCount)));
END //
DELIMITER ;
...
-- Intermediate 2 stores 1 row with the customer's stations' building name (from intermediate_1), and that building's tags and description
    ...
    INSERT INTO intermediate_2 (`description`)
    SELECT Building.`description` WHERE Building.buildingName = intermediate_1.buildingName;

--  Inserts ordered result by joining the 2 intermediate tables above based on their common building name

    INSERT INTO cus_current_information_basic_result(stationName, buildingName, tags, `description`, balance) VALUES (
...
--  Intermediate_2 stores the manager username and full name for managers managing trucks in Intermediate_1 
    ...
    DROP TABLE IF EXISTS intermediate_2;
    CREATE TABLE intermediate_2(managerUsername varchar(55), managerName varchar(100));
    INSERT INTO intermediate_2(managerUsername, managerName) 
    SELECT `User`.username, `User`.firstName + ' ' + `User`.lastName AS managerName
...
--  Intermediate_3 stores each of the food trucks in Intermediate_1 along with a list of their food names
    ...
    INSERT INTO intermediate_3(foodTruckName, foodNames) VALUES(
    (SELECT intermediate_1.foodTruckName FROM intermediate_1),
    (SELECT GROUP_CONCAT(DISTINCT(MenuItem.foodName) SEPARATOR ',') FROM MenuItem
...
-- here I am just making a table to keep track of all the variables
	...
	IF (SELECT foodPrice FROM add_result)*
    (SELECT purchaseQuantity FROM add_result) <= 
    (SELECT customerbalance FROM add_result) THEN
